<script lang="ts">
import type { MasterPartName } from '@/types/master.type';
...
const defprops = {
    masteritems: {type: Array as PropType<MasterPartName[]>, default: undefined, required: true},
}
...
export default defineComponent({
    ...
    setup(props, {emit}){
        ...
        const mstitems = ref<MasterPartName[]|undefined>();
        ...
        const fnFetchmaster = async():Promise<MasterPartName[]> =>{
            return new Promise(async(res, rej)=>{
                isOpendialogFetchingdata.value = true;
                const result = await axios?.get<string,any>(`/Master/GetMasterPartname`).catch((error)=>{
                    alert('Fail', `Code : ${result.status} Message : ${result.data}`)
                })
                isOpendialogFetchingdata.value = false;
                if(result.status == 200)
                {
                    res(result.data as MasterPartName[]);
                }
                rej([]);
            })
        }
        ...
        const selectedMasterObj = ref<MasterPartName>({id:1, name:''});
        ...
        const fnClickRefresh = async()=>{
            ...
            masterstore.mastervalue!.partname = [...master];
            //Reassign reference to variable.
            mstitems.value = masterstore.mastervalue!.partname;
        }

        const fnClickEdit = (item:MasterPartName)=>{
            isEdit.value = true;
            selectedMasterObj.value = {...item};
            isOpendialog.value = true;
        }

        const fnClickDelete = async(item:MasterPartName)=>{
            const result = await confirmation('Confirmation' ,'Do you want to delete this item?')

            if(result.isconfirm)
            {
                const query = `/Master/DeleteMasterPartname?id=${item.id}`
                let res = await axios?.delete<MasterPartName, any>(query,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
        ...
        const fnSubmit = async()=>
        {
            ...
            if(isEdit.value)
            {
                ...
                const query = `/Master/UpdateMasterPartname?id=${item.id}`
                let res = await axios?.put<MasterPartName, any>(query, selectedMasterObj.value,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
                    await alert(`Error`, `Code : ${res.status} Message : ${res.data}`)
                    return;
                })
            ...
            {
                let res = await axios?.post<MasterPartName, any>("/Master/AddMasterPartname", selectedMasterObj.value,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
                    await alert(`Error`, `Code : ${res.status} Message : ${res.data}`)
                    return;
                })
