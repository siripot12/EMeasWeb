...
const defprops = {
    masteritems: {type: Array as PropType<MasterMeasureMode[]>, default: undefined, required: true},
}
...
export default defineComponent({
    ...
    setup(props, {emit}){
        ...
        const mstitems = ref<MasterMeasureMode[]|undefined>();
        ...
        const fnFetchmaster = async():Promise<MasterMeasureMode[]> =>{
            return new Promise(async(res, rej)=>{
                isOpendialogFetchingdata.value = true;
                const result = await axios?.get<string,any>(`/Master/GetMasterMeasuremode`).catch((error)=>{
                    alert('Fail', `Code : ${result.status} Message : ${result.data}`)
                })
                isOpendialogFetchingdata.value = false;
                if(result.status == 200)
                {
                    res(result.data as MasterMeasureMode[]);
                }
                rej([]);
            })
        }
        ...
        const selectedMasterObj = ref<MasterMeasureMode>({id:1, name:''});
        ...
        const fnClickRefresh = async()=>{
            ...
            masterstore.mastervalue!.measuremode = [...master];
            //Reassign reference to variable.
            mstitems.value = masterstore.mastervalue?.measuremode;
        }

        const fnClickEdit = (item:MasterMeasureMode)=>{
            isEdit.value = true;
            selectedMasterObj.value = {...item};
            isOpendialog.value = true;
        }

        const fnClickDelete = async(item:MasterMeasureMode)=>{
            const result = await confirmation('Confirmation' ,'Do you want to delete this item?')

            if(result.isconfirm)
            {
                const query = `/Master/DeleteMasterMeasuremode?id=${item.id}`
                let res = await axios?.delete<MasterMeasureMode, any>(query,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
        ...
        const fnSubmit = async()=>
        {
            ...
            if(isEdit.value)
            {
                ...
                let res = await axios?.put<MasterMeasureMode, any>("/Master/UpdateMasterMeasuremode", selectedMasterObj.value,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
                    await alert(`Error`, `Code : ${res.status} Message : ${res.data}`)
                    return;
                })
            ...
            {
                let res = await axios?.post<MasterMeasureMode, any>("/Master/AddMasterMeasuremode", selectedMasterObj.value,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
                    await alert(`Error`, `Code : ${res.status} Message : ${res.data}`)
                    return;
                })
