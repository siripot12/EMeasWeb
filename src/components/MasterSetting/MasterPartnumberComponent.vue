<script lang="ts">
import type { MasterPartnumber } from '@/types/master.type';
...
const defprops = {
    masteritems: {type: Array as PropType<MasterPartnumber[]>, default: undefined, required: true},
}
...
export default defineComponent({
    ...
    setup(props, {emit}){
        ...
        const mstitems = ref<MasterPartnumber[]|undefined>();
        ...
        const fnFetchmaster = async():Promise<MasterPartnumber[]> =>{
            return new Promise(async(res, rej)=>{
                isOpendialogFetchingdata.value = true;
                const result = await axios?.get<string,any>(`/Master/GetMasterPartnumber`).catch((error)=>{
                    alert('Fail', `Code : ${result.status} Message : ${result.data}`)
                })
                isOpendialogFetchingdata.value = false;
                if(result.status == 200)
                {
                    res(result.data as MasterPartnumber[]);
                }
                rej([]);
            })
        }
        ...
        const selectedMasterObj = ref<MasterPartnumber>({id:1, name:''});
        ...
        const fnClickAdd = ()=>{
            ...
            selectedMasterObj.value = {id:iditem.value[0], name:''}
        ...
        const fnClickRefresh = async()=>{
            ...
            masterstore.mastervalue!.partnumber = [...master];
            //Reassign reference to variable.
            mstitems.value = masterstore.mastervalue?.partnumber;
        }

        const fnClickEdit = (item:MasterPartnumber)=>{
            isEdit.value = true;
            selectedMasterObj.value = {...item};
            isOpendialog.value = true;
        }

        const fnClickDelete = async(item:MasterPartnumber)=>{
            const result = await confirmation('Confirmation' ,'Do you want to delete this item?')

            if(result.isconfirm)
            {
                const query = `/Master/DeleteMasterPartnumber?id=${item.id}`
                let res = await axios?.delete<MasterPartnumber, any>(query,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
        ...
        const fnSubmit = async()=>
        {
            ...
            if(isEdit.value)
            {
                ...
                const query = `/Master/UpdateMasterPartnumber?id=${item.id}`
                let res = await axios?.put<MasterPartnumber, any>(query, selectedMasterObj.value,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
                    await alert(`Error`, `Code : ${res.status} Message : ${res.data}`)
                    return;
                })
            ...
            {
                let res = await axios?.post<MasterPartnumber, any>("/Master/AddMasterPartnumber", selectedMasterObj.value,{headers:{Accept:'application/json', "Content-Type":'application/json'}})
                .catch(async(error)=>{
                    await alert(`Error`, `Code : ${res.status} Message : ${res.data}`)
                    return;
                })
